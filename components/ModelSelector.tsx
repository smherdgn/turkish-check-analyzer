        ...
        {models.map((model) => {
          ...
          return (
            ...
            className={`
              ...
              ${selectedModels.includes(cleanName) ? `bg-green-50 border-[${GARANTI_GREEN}] ring-2 ring-[${GARANTI_GREEN}]` : 'bg-gray-50 border-gray-200'}
            `}
          >
            <input
              type="checkbox"
              id={`model-${cleanName}`}
              name={cleanName}
              checked={selectedModels.includes(cleanName)}
              onChange={(e) => onChange(cleanName, e.target.checked)}
              disabled={disabled}
              className="form-checkbox h-5 w-5 rounded text-[#1EA48A] focus:ring-[#1EA48A] border-gray-300 disabled:opacity-50"
            />
            <div className="flex flex-col">
              <span className={`text-sm font-medium ${selectedModels.includes(cleanName) ? `text-[${GARANTI_GREEN}]` : 'text-gray-800'}`}>
                {model.name.trim()}
              </span>
